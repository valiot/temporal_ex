...
- [Improvement] Moved to `grpc` 1.0.5 (from 0.11.5) by way of `temporalio` 1.63.5, which lifts the `~> 0.11.0` pin that blocked 0.2.7; `grpc_core` 1.0.5 arrives with it and `protobuf` goes 0.16.1 → 0.17.0. This clears all four advisories 0.2.7 documented — CVE-2026-48853 (CRITICAL), CVE-2026-48599 / CVE-2026-53430 / CVE-2026-48854 (HIGH) — so `mix hex.audit` no longer reports `grpc` at all. grpc 1.0 also splits the server side out of the client package, which drops `cowboy`, `ranch`, `flow`, `gen_stage`, `hpax` and `mint` from this client-only dependency tree (28 → 23 locked packages).
- [Improvement] `gun` is now a direct dependency. grpc 1.0 makes its transports optional while still defaulting to the Gun adapter, so a client that says nothing would boot with no HTTP/2 transport at all. grpc 1.0.5 requires `gun ~> 2.4.0`, so this is a downgrade from 2.6.0 to 2.4.1; the advisory `hex.audit` reports on it (GHSA-w4f7-4cxr-rv3c, MEDIUM) was already open at 2.6.0 and has no fixed release, so exposure is unchanged.
- [Fix] Adapted `TemporalEx.Client` to grpc 1.0's connection model, in which gun is owned by a per-channel `GRPC.Client.Adapters.Gun.ConnectionProcess` GenServer rather than by whoever opened the channel. Three consequences: `TemporalEx.Application` no longer starts `GRPC.Client.Supervisor` (grpc's own application does, and starting it twice is a hard boot failure); the `{:gun_up, …}` / `{:gun_down, …}` `handle_info` clauses are gone, since those messages now go to the adapter's process and the clauses were dead code — dropping a dead channel is the mid-RPC retry path's job, as it has been since 0.2.2; and `invoke_rpc/4` catches the `GenServer.call` exit (`:noproc` / `:normal`) that grpc 1.0 raises when the connection process is already a corpse, re-shaping it into the `:down: …` form the 0.2.2 reconnect-and-retry-once path understands. A `:timeout` exit still propagates, because the server may already have acted on the request. `channel_conn_pid/1` now identifies the connection process rather than the gun pid, which remains a stable per-channel identity for `same_channel?`. No public API change; test doubles speak the 1.0 adapter protocol (`$gen_call` rather than gun's `$gen_cast`).
