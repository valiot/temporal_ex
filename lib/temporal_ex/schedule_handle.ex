defmodule TemporalEx.ScheduleHandle do
  ...
  """
  @spec pause(t(), keyword()) :: :ok | {:error, Error.t()}
  def pause(%__MODULE__{} = handle, opts \\ []) when is_list(opts) do
    {note, rest} = Keyword.pop(opts, :note, "paused")
    patch(handle, Keyword.put(rest, :pause, note))
  end
  ...
  """
  @spec unpause(t(), keyword()) :: :ok | {:error, Error.t()}
  def unpause(%__MODULE__{} = handle, opts \\ []) when is_list(opts) do
    {note, rest} = Keyword.pop(opts, :note, "unpaused")
    patch(handle, Keyword.put(rest, :unpause, note))
  end
